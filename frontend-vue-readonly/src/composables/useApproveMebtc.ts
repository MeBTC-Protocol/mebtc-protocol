...
export function useApproveMebtc() {
  ...
  async function approve(amount: bigint) {
    ...
    try {
      ...
      const tx = await mebtc.approve(ADDRESSES.stakeVault, amount)
      lastTx.value = tx.hash
      await tx.wait()
      triggerRefresh('approve-mebtc')
    } catch (e: any) {
  ...
  return { busy, error, lastTx, approveMax, approveExact }
}
